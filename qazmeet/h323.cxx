...
#ifdef _WIN32
PluginLoaderStartup2  QazMeet::pluginLoader;
H323PluginCodecManager * QazMeet::plugmgr=NULL;
#endif
  ...
  : conferenceManager(_conferenceManager)
{
  ...
  QazMeet::Current().LoadPluginMgr();
  QazMeet::Current().pluginLoader.OnStartup();
#endif
}
...
MCUH323EndPoint::~MCUH323EndPoint()
{
  ...
  QazMeet::Current().RemovePluginMgr();
  QazMeet::Current().pluginLoader.OnShutdown();
#endif
}
...
void MCUH323EndPoint::Initialise(PConfig & cfg)
{
  ...
  SetLocalUserName(QazMeet::Current().GetServerId());
...
BOOL MCUH323EndPoint::OnConnectionCreated(MCUH323Connection * conn)
{
  ...
  Registrar *registrar = QazMeet::Current().GetRegistrar();
...
PString MCUH323EndPoint::GetConferenceOptsJavascript(Conference & c)
{
  ...
  r << "conf=[[" //l1&l2 open
    ...
    << "," << QazMeet::vmcfg.bfw                                        // [0][1]  = base frame width
    << "," << QazMeet::vmcfg.bfh                                        // [0][2]  = base frame height
  ...
  r << "," << "\"" << QazMeet::GetScaleFilterName(scaleFilterType) << "\""; // [0][10] = yuv resizer filter mode
...
MCUJSON* MCUH323EndPoint::GetVideoMixerConfiguration(MCUVideoMixer * mixer, int number)
{
  ...
  VMPCfgSplitOptions & split=QazMeet::vmcfg.vmconf[n].splitcfg;
  VMPCfgOptions      * p    =QazMeet::vmcfg.vmconf[n].vmpcfg;
...
PString MCUH323EndPoint::GetAddressBookOptsJavascript()
{
  ...
  MCUAbookList & abookList = QazMeet::Current().GetRegistrar()->GetAbookList();
  int i = 0;
...
int MCUH323EndPoint::SetMemberVideoMixer(Conference & conference, ConferenceMember * member, int newMixerNumber)
{
  ...
  QazMeet::Current().HttpWriteCmdRoom(cmd, conference.GetNumber());
...
PString MCUH323EndPoint::RoomCtrlPage(const PString room)
{
 ...
 page << "<div id='cb1' name='cb1' class='input-append'>&nbsp;</div>"
   ...
   << "</div>"
 ;
 EndPage(page,QazMeet::Current().GetHtmlCopyright());
 return page;
}
...
PString MCUH323EndPoint::SetRoomParams(const PStringToString & data)
{
  ...
  if(data.Contains("otfc"))
    return (QazMeet::Current().OTFControl(data) ? "OK" : "FAIL");
  ...
  QazMeet::Current().HttpWriteEventRoom("MCU Operator connected",room);
  ...
  for(MCUConferenceList::shared_iterator it = conferenceList.begin(); it != conferenceList.end(); ++it)
  {
    ...
    for(MCUMemberList::shared_iterator it = memberList.begin(); it != memberList.end(); ++it)
    {
      ...
      if(member->videoMixer!=NULL)
      {
        ...
        output << hdr << "Video Mixer Layout ID: " << QazMeet::vmcfg.vmconf[n].splitcfg.Id << "\n"
          << hdr << "Video Mixer Layout capacity: " << dec << QazMeet::vmcfg.vmconf[n].splitcfg.vidnum << hex << "\n";
        for(MCUVMPList::shared_iterator it = member->videoMixer->vmpList.begin(); it != member->videoMixer->vmpList.end(); ++it)
        {
          ...
          VMPCfgOptions & vmpcfg = QazMeet::vmcfg.vmconf[member->videoMixer->GetLayout()].vmpcfg[vmp->n];
    ...
    for(MCUVideoMixerList::shared_iterator it = videoMixerList.begin(); it != videoMixerList.end(); ++it, ++number)
    {
      ...
      output << "  Layout ID: "       << QazMeet::vmcfg.vmconf[n].splitcfg.Id << "\n"
             << "  Layout capacity: " << QazMeet::vmcfg.vmconf[n].splitcfg.vidnum << "\n"
             << mixer->GetFrameStoreMonitorList();

      for(MCUVMPList::shared_iterator it = mixer->vmpList.begin(); it != mixer->vmpList.end(); ++it)
      {
        VideoMixPosition *vmp = *it;
        VMPCfgOptions & vmpcfg = QazMeet::vmcfg.vmconf[mixer->GetLayout()].vmpcfg[vmp->n];
...
PString MCUH323EndPoint::Invite(PString room, PString memberName)
{
  ...
  {
    MCUSipEndPoint * sep = QazMeet::Current().GetSipEndpoint();
  ...
  {
    MCURtspServer *rtsp = QazMeet::Current().GetRtspServer();
...
//    QazMeet::Current().HttpWriteEventRoom(msg, room);
    return callToken;
}
...
//  if (roomToJoin.IsEmpty() && !setupPDU.GetQ931().GetCalledPartyNumber(roomToJoin) && roomToJoin.IsEmpty()) {
  ...
  if (roomToJoin.IsEmpty()) 
    roomToJoin = QazMeet::Current().GetDefaultRoomName();
  ...
  : H323Connection(_ep, callReference), ep(_ep), isMCU(FALSE)
{
  ...
  if(requestedRoom == "")
    requestedRoom = QazMeet::Current().GetDefaultRoomName();

  localPartyName = requestedRoom;
  localDisplayName = QazMeet::Current().GetServerId();
  if(!ep.IsRegisteredWithGatekeeper())
  {
    localAliasNames.RemoveAll();
//    localAliasNames.AppendString(QazMeet::Current().GetName());
...
/*
  if(requestedRoom != "")
  {
    ...
    QazMeet::Current().HttpWriteEventRoom(event, requestedRoom);
  }
*/
}
...
BOOL MCUH323Connection::CheckVFU()
{
  ...
  if(now < vfuBeginTime + PTimeInterval(vfuInterval*1000))
  {
    ...
    if(vfuCount > vfuLimit)
    {
      ...
      QazMeet::Current().HttpWriteEventRoom("<font color=red>"+event+"</font>", requestedRoom);
    }
    vfuBeginTime = now;
    vfuCount = 1;
  }
  ...
  if(code != "" && MCUConfig("Control Codes").HasKey(code))
  {
    ...
    if(codeConferenceMember != NULL)
    {
      if(codeAction == "close")
        ...
        codeConferenceMember->muteMask&=~1;
      codeMsg << "</font>";
      QazMeet::Current().HttpWriteEvent(codeMsg);
      //QazMeet::Current().HttpWriteEventRoom(codeMsg, conference->GetNumber());
    }
  } else {
...
{
  PFilePath fn = QazMeet::Current().GetConnectingWAVFile();
  ...
  switch(welcomeState) {
    ...
    case PlayingConnecting:
      PlayWelcomeFile(FALSE, fn);
      //PlayWelcomeFile(QazMeet::Current().GetConnectingWAVFile(fn), fn);
      break;
...
MCUConnection_ConferenceMember::MCUConnection_ConferenceMember(Conference * _conference, const PString & _memberName, const PString & _callToken, BOOL _isMCU)
  : ConferenceMember(_conference), ep(QazMeet::Current().GetEndpoint())
{
...
void MCUConnection_ConferenceMember::SendUserInputIndication(const PString & str)
{
  ...
  QazMeet::Current().HttpWriteEvent(msg);
...
void MCUH323Connection::LogCall(const BOOL accepted)
{
  ...
  if(accepted)
  {
    ...
    QazMeet::Current().LogMessage(timeStream + stringStream	+ " duration:" + connectionDuration);
  }
  else
    QazMeet::Current().LogMessage(timeStream + " Call denied:" + stringStream);
}
...
void GatekeeperMonitor::Main()
{
  for(;;)
  {
    ...
    {
      MCUTRACE(1, event);
      QazMeet::Current().HttpWriteEvent("<font color=blue>"+event+"</font>");
    }
