...
PString Conference::SaveTemplate(PString tplName)
{
  ...
  for(MCUVideoMixerList::shared_iterator it = videoMixerList.begin(); it != videoMixerList.end(); ++it)
  {
    ...
    VMPCfgSplitOptions & o = QazMeet::vmcfg.vmconf[n].splitcfg;
          ...
          if(mixer)
          {
            ...
            { 
              ...
              if(!layoutName.IsEmpty())
              {
                ...
                if(layoutNumber < QazMeet::vmcfg.vmconfs)
                  layoutNameRef = QazMeet::vmcfg.vmconf[layoutNumber].splitcfg.Id;
                if((layoutName == layoutNameRef) && (layoutNumber < QazMeet::vmcfg.vmconfs))
                  mixer->MyChangeLayout(layoutNumber);
                else
                {
                  for(unsigned i=0;i<QazMeet::vmcfg.vmconfs;i++)
                  {
                    if(!strcmp((const char*)QazMeet::vmcfg.vmconf[i].splitcfg.Id,(const char*)layoutName))
                    {
...
//              member->kOutputGain=(float)pow(10.0,((float)member->kOutputGainDB)/20.0);
            ...
            QazMeet::Current().GetEndpoint().SetMemberVideoMixer(*this, member, v[4].AsInteger());
...

  {
    ...
    for(MCUMemberList::shared_iterator it = memberList.begin(); it != memberList.end(); ++it)
    {
      ...
      if(validatedMembers.GetStringsIndex(name) == P_MAX_INDEX) // remove unwanted members
      {
        ...
        if(member->IsOnline())
        {
          MCUH323Connection * conn = QazMeet::Current().GetEndpoint().FindConnectionWithLock(member->GetCallToken());
          if(conn)
          {
