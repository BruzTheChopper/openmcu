                                              ...
                                              BOOL /*endFrame*/)
{
  ...
  {
    PTRACE(1, "QazMeet only supports full frame writes");
    return FALSE;
  }
  ...
  : width(_w), height(_h), frame_size(_w*_h*3/2)
{
  ...
  unsigned bgw, bgh;
  void *bg = QazMeet::Current().GetBackgroundPointer(bgw, bgh);
  if(bg)
  ...
  void *logo = QazMeet::Current().GetLogoFramePointer(logo_width, logo_height);
  if(logo)
...
void MCUPrintSubtitles(VideoMixPosition & vmp, void * buffer, unsigned int fw, unsigned int fh, unsigned int ft_properties, unsigned layout)
{
  ...
  unsigned bgcolor = QazMeet::vmcfg.vmconf[layout].vmpcfg[vmp.n].label_bgcolor;
...
# ifdef TRUETYPE_FONT_DIR
    ft_error = FT_New_Face(ft_library,PString(TRUETYPE_FONT_DIR)+PATH_SEPARATOR+QazMeet::vmcfg.fontfile,0,&ft_face);
# else
    ft_error = FT_New_Face(ft_library,QazMeet::vmcfg.fontfile,0,&ft_face);
# endif
  if (!ft_error)
  {
    ft_use_kerning=FT_HAS_KERNING(ft_face);
    PTRACE(3,"FreeType\tTruetype font " << QazMeet::vmcfg.fontfile << " loaded with" << (ft_use_kerning?"":"out") << " kerning");
  }
  PTRACE_IF(1,ft_error,"FreeType\tCould not load truetype font: " << QazMeet::vmcfg.fontfile);
}
...
MCUSubtitles * MCURenderSubtitles(VideoMixPosition & vmp, unsigned fw, unsigned fh, unsigned ft_properties, unsigned layout)
{
  ...
  VMPCfgSplitOptions & split = QazMeet::vmcfg.vmconf[layout].splitcfg;
  VMPCfgOptions & vmpcfg = QazMeet::vmcfg.vmconf[layout].vmpcfg[vmp.n];

  if((fw < 2) || (fh < 2)) return st;

  if(fw < MCUSubsCalc(fw * QazMeet::vmcfg.bfw / vmpcfg.width, split.minimum_width_for_label)) return st;
...
BOOL MCUSimpleVideoMixer::ReadMixedFrame(VideoFrameStoreList & srcFrameStores, void * buffer, int width, int height, PINDEX & amount)
{
  ...
  for(unsigned i = 0; i < QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum; i++)
  {
    VMPCfgOptions & vmpcfg = QazMeet::vmcfg.vmconf[specialLayout].vmpcfg[i];
    ...
    // grid
    if(QazMeet::vmcfg.vmconf[specialLayout].vmpcfg[i].border)
    {
      if(px != 0)
...
{
  VMPCfgOptions & vmpcfg=QazMeet::vmcfg.vmconf[specialLayout].vmpcfg[vmp.n];
    ...
    if(vmp->vmpbuf_index == -1 || vmp->shows_logo)
    {
      if(vmpList.Erase(vmp_it))
      {
        if(vmp->vmpbuf_index == -1)
        {
          ...
          void *buffer = QazMeet::Current().GetPreMediaFrame(width, height);
...
void MCUSimpleVideoMixer::WriteEmptyFrame(VideoMixPosition & vmp)
{
  ...
  void * buffer = QazMeet::Current().GetEmptyFramePointer(width, height);
...
void MCUSimpleVideoMixer::WriteOfflineFrame(VideoMixPosition & vmp)
{
  ...
  unsigned width, height, options;
  void * buffer = QazMeet::Current().GetOfflineFramePointer(width, height);
...
void MCUSimpleVideoMixer::WriteNoVideoFrame(VideoMixPosition & vmp)
{
  ...
  unsigned width, height, options;
  void * buffer = QazMeet::Current().GetNoVideoFramePointer(width, height);
...
void MCUSimpleVideoMixer::Shuffle()
{
  ...
  unsigned & vidnum = QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum;
...
int MCUSimpleVideoMixer::VMPListFindEmptyIndex()
{
  ...
  for(int i=0; (unsigned)i<QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum; i++)
  {
...
MCUVMPList::shared_iterator MCUSimpleVideoMixer::VMPCreator(ConferenceMember * member, int n, int type)
{
  ...
  if((unsigned)n >= QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum) return it;
...
void MCUSimpleVideoMixer::RemoveVideoSource(ConferenceMemberId id, ConferenceMember & mbr)
{
  ...
  if (newsL!=specialLayout || QazMeet::vmcfg.vmconf[newsL].splitcfg.reallocate_on_disconnect)
  {
    specialLayout=newsL;
    ReallocatePositions();
  }
...
void MCUSimpleVideoMixer::SetPositionType(int pos, int type)
{
  ...
  if((unsigned)pos>=QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum) return;
  if(type==1) return;
...
void MCUSimpleVideoMixer::MyChangeLayout(unsigned newLayout)
{
  ...
  if(newLayout >= QazMeet::vmcfg.vmconfs) return;
  int newCount = QazMeet::vmcfg.vmconf[newLayout].splitcfg.vidnum;
...
void MCUSimpleVideoMixer::Exchange(int pos1, int pos2)
{
  ...
  unsigned layoutCapacity = QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum;
...
int MCUSimpleVideoMixer::GetMostAppropriateLayout(unsigned n)
{
  ...
  {
    unsigned & vidnum = QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;
    if(QazMeet::vmcfg.vmconf[i].splitcfg.mode_mask & 1)
    {
  ...
  : frames(_frames), allocated(FALSE)
{
  ...
  for(unsigned i=0;i<QazMeet::vmcfg.vmconfs;i++) if(QazMeet::vmcfg.vmconf[i].splitcfg.mode_mask&1)
  { if(QazMeet::vmcfg.vmconf[i].splitcfg.vidnum==_frames) { specialLayout=i; break; }
    else if((QazMeet::vmcfg.vmconf[i].splitcfg.vidnum>_frames)&&(QazMeet::vmcfg.vmconf[i].splitcfg.vidnum<matchV))
    { matchV=QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;  specialLayoutMaxOfMatched=(int)i; }
    else if((int)QazMeet::vmcfg.vmconf[i].splitcfg.vidnum>maxV)
    { maxV=QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;  specialLayoutMaxFrames=(int)i; }
  }
  if(specialLayout==-1)
...
BOOL TestVideoMixer::AddVideoSource(ConferenceMemberId id, ConferenceMember & mbr)
{
  ...
  unsigned _f = frames; if (_f==0) _f=QazMeet::vmcfg.vmconf[specialLayout].splitcfg.vidnum;

  for (unsigned i = 0; i < _f; ++i) {
    o = &(QazMeet::vmcfg.vmconf[specialLayout].vmpcfg[i]);
  ...
  : MCUSimpleVideoMixer()
{
  ...
  int specialLayoutMaxOfMatched=-1; unsigned matchV=99999;
  for(unsigned i=0;i<QazMeet::vmcfg.vmconfs;i++) if(QazMeet::vmcfg.vmconf[i].splitcfg.mode_mask&1)
  { if(QazMeet::vmcfg.vmconf[i].splitcfg.vidnum==1) { specialLayout=i; break; }
    else if((QazMeet::vmcfg.vmconf[i].splitcfg.vidnum>1)&&(QazMeet::vmcfg.vmconf[i].splitcfg.vidnum<matchV))
    { matchV=QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;  specialLayoutMaxOfMatched=(int)i; }
    else if((int)QazMeet::vmcfg.vmconf[i].splitcfg.vidnum>maxV)
    { maxV=QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;  specialLayoutMaxFrames=(int)i; }
  }
