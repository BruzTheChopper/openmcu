...
void ConferenceManager::OnDestroyConference(Conference * conference)
{
  ...
  QazMeet::Current().HttpWriteCmdRoom("notice_deletion(1,'" + jsName + "')", number);
  ...
  //QazMeet::Current().HttpWriteCmdRoom("notice_deletion(2,'" + jsName + "')", number);
  QazMeet::Current().HttpWriteCmdRoom("notice_deletion(3,'" + jsName + "')", number);
  ...
  QazMeet::Current().HttpWriteCmdRoom("notice_deletion(4,'" + jsName + "')", number);
  QazMeet::Current().HttpWriteCmdRoom("notice_deletion(5,'" + jsName + "')", number);
}
...
int ConferenceMonitor::Perform(Conference * conference)
{
  ...
  // autodial
  if(QazMeet::Current().autoDialDelay != 999999) //disable
  {
    if((conference->dialCountdown--) <= 0)
    {
      ...
      conference->dialCountdown = QazMeet::Current().autoDialDelay;
    }
  }
  ...
  : manager(_manager), listID(_listID), guid(_guid), number(_number), name(_name)
{
  ...
  muteNewUsers = FALSE;
  pipeMember = NULL;
  dialCountdown = QazMeet::Current().autoDialDelay;
  SetMasterVolumeDB(0);
...
{
  PDirectory pd(QazMeet::Current().vr_ffmpegDir);
  PInt64 t, f;
  DWORD cs;
  ...
  BOOL result = ((f>>20) >= QazMeet::Current().vr_minimumSpaceMiB);
  if(!result) QazMeet::Current().HttpWriteEvent("<b><font color='red'>Insufficient disk space</font>: Video Recorder DISABLED</b>");
...
BOOL Conference::StartRecorder()
{
  ...
  if(!RecorderCheckSpace())
    return FALSE;
  if(!PDirectory::Exists(QazMeet::Current().vr_ffmpegDir))
  {
    PTRACE(1, trace_section << "Recorder failed to start (check recorder directory)");
    QazMeet::Current().HttpWriteEventRoom("Recorder failed to start (check recorder directory)", number);
    return FALSE;
  }
  ...
  QazMeet::Current().HttpWriteEventRoom("video recording started", number);
  QazMeet::Current().HttpWriteCmdRoom(QazMeet::Current().GetEndpoint().GetConferenceOptsJavascript(*this), number);
  QazMeet::Current().HttpWriteCmdRoom("build_page()", number);
  return TRUE;
}
...
BOOL Conference::StopRecorder()
{
  ...
  QazMeet::Current().HttpWriteEventRoom("video recording stopped", number);
  QazMeet::Current().HttpWriteCmdRoom(QazMeet::Current().GetEndpoint().GetConferenceOptsJavascript(*this), number);
  QazMeet::Current().HttpWriteCmdRoom("build_page()", number);
  return TRUE;
}
  ...
  if(!memberToAdd->IsSystem())
  {
    ...
    for(PINDEX i = 0; ; i++)
    {
      ...
      if(MCUConfig("Parameters").GetBoolean(RejectDuplicateNameKey, FALSE))
      {
        ...
        QazMeet::Current().HttpWriteEventRoom(msg, number);
...
void Conference::WriteMemberAudioLevel(ConferenceMember * member, int audioLevel, int tint)
{
  ...
  if(member->audioCounter>1999) //2s
  {
    ...
    if((member->previousAudioLevel != member->audioLevelIndicator)||((member->audioLevelIndicator!=0) && ((member->audioCounter&255)==0)))
    {
      ...
      QazMeet::Current().HttpWriteCmdRoom(msg,number);
  ...
  : conference(_conference)
{
  ...
  visible = FALSE;
  isMCU = FALSE;
  id = (ConferenceMemberId)QazMeet::GetUniqueMemberID();
  channelMask = 0;
  audioLevel = 0;
  audioCounter = 0;
...
void ConferenceMember::Dial(int _autoDial)
{
  ...
  PWaitAndSignal m(dialMutex);
  autoDial = _autoDial;
  if((autoDial && (QazMeet::Current().autoDialDelay < 20)) || IsOnline())
    return;
  MCUH323EndPoint & ep = QazMeet::Current().GetEndpoint();
...
void ConferenceMember::SendRoomControl(int state)
{
  ...
  if(IsOnline())
  {
    if(state == 1)
    {
      ...
      QazMeet::Current().HttpWriteEventRoom(msg, conference->GetNumber());
    } else {
  ...
  msg  << (state && IsOnline())
       ...
       << "," << resizerRule
       << ")";
  QazMeet::Current().HttpWriteCmdRoom(msg, conference->GetNumber());
}
