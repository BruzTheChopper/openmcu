...
{
  return QazMeet::Current().GetEndpoint().GetMonitorText();
}

PCREATE_SERVICE_MACRO_BLOCK(RoomStatus,P_EMPTY,P_EMPTY,block)
{
  return QazMeet::Current().GetEndpoint().GetRoomStatus(block);
}

PCREATE_SERVICE_MACRO_BLOCK(RoomList,P_EMPTY,P_EMPTY,block)
{
  return QazMeet::Current().GetEndpoint().GetRoomList(block);
}
...
)
{
  ...
  p << "<html>";
    ...
    p << "<hr><i>"
      << QazMeet::Current().GetName() << " "  << QazMeet::Current().GetVersion(TRUE)
      << " (" << QazMeet::Current().GetOSName() << " " << QazMeet::Current().GetOSVersion() << ") "
      << " Server at "
      << ip << " Port " << port
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  // MCU Server Id
  s << StringField(ServerIdKey, JsLocal("server_id"), QazMeet::Current().GetServerId(), 250);
  ...
  EndPage(html_end,QazMeet::Current().GetHtmlCopyright());
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  if(sect.GetSize() == 1)
    sect.AppendString(sectionPrefix+QazMeet::Current().GetDefaultRoomName());
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  {
    s << NewRowInput("1");
    s << StringItem("1", QazMeet::Current().GetDefaultRoomName());
  }
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  int scaleFilterType = QazMeet::Current().GetScaleFilterType();
  s << SelectField(VideoScaleFilterKey, VideoScaleFilterKey, QazMeet::GetScaleFilterName(scaleFilterType), MCUScaleFilterNames);
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  s << StringItem(name, QazMeet::Current().GetDefaultRoomName(), 120);
    ...
    : TablePConfigPage(app,title,section,auth)
{
  ...
  if(keys.GetSize() == 1)
    keys.AppendString(QazMeet::Current().GetDefaultRoomName());
...
WelcomePage::WelcomePage(QazMeet & _app, PHTTPAuthority & auth)
...
BOOL WelcomePage::OnPOST(PHTTPServer & server, const PURL & url, const PMIMEInfo & info, const PStringToString & data, const PHTTPConnectionInfo & connectInfo)
{
  ...
  server.flush();
  //QazMeet::Current().RemovePreMediaFrame();
...
BOOL WelcomePage::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  PTime now;
  PTimeInterval upTime = now - QazMeet::Current().GetStartTime();
  ...
  shtml << "<br><b>Monitor Text (<span style='cursor:pointer;text-decoration:underline' onclick='javascript:{if(document.selection){var range=document.body.createTextRange();range.moveToElementText(document.getElementById(\"monitorTextId\"));range.select();}else if(window.getSelection){var range=document.createRange();range.selectNode(document.getElementById(\"monitorTextId\"));window.getSelection().addRange(range);}}'>select all</span>)</b><div style='padding:5px;border:1px dotted #595;width:100%;height:auto;max-height:300px;overflow:auto'><pre style='margin:0px;padding:0px' id='monitorTextId'>"
        ...
        << PRODUCT_NAME_TEXT << " REVISION " << MCU_STRINGIFY(GIT_REVISION) << "\n"
              ...
              << "Program: "          << QazMeet::Current().GetProductName() << "\n"
              << "Version: "          << QazMeet::Current().GetVersion(TRUE) << "\n"
              << "Manufacturer: "     << QazMeet::Current().GetManufacturer() << "\n"
              << "OS: "               << QazMeet::Current().GetOSClass() << " " << QazMeet::Current().GetOSName() << "\n"
              << "OS Version: "       << QazMeet::Current().GetOSVersion() << "\n"
              << "Hardware: "         << QazMeet::Current().GetOSHardware() << "\n"
              << "Compilation date: " << QazMeet::Current().GetCompilationDate().AsString(timeFormat, PTime::GMT) << "\n"
              << "Start Date: "       << QazMeet::Current().GetStartTime().AsString(timeFormat, PTime::GMT) << "\n"
  ...
  EndPage(shtml,QazMeet::Current().GetHtmlCopyright());
  { PStringStream message; PTime now; message
      << "HTTP/1.1 200 OK\r\n"
...
MainStatusPage::MainStatusPage(QazMeet & _app, PHTTPAuthority & auth)
    ...
    app(_app)
{
  ...
  html << "<p>"
       ...
       << "<p>";
         ...
         EndPage(html,QazMeet::Current().GetHtmlCopyright());
  string = html;
}
...
BOOL MainStatusPage::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  if(data.Contains("start"))
    body = QazMeet::Current().GetEndpoint().GetRoomStatusJSStart();
  else
    body = QazMeet::Current().GetEndpoint().GetRoomStatusJS();
...
InvitePage::InvitePage(QazMeet & _app, PHTTPAuthority & auth)
                          ...
                          PHTML & msg)
{
  ...
  if (room.IsEmpty() || address.IsEmpty()) {
    ...
    html << html_invite;
    EndPage(html,QazMeet::Current().GetHtmlCopyright()); msg = html;
    return TRUE;
  }
...
JpegFrameHTTP::JpegFrameHTTP(QazMeet & _app, PHTTPAuthority & auth)
...
BOOL JpegFrameHTTP::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  if(t1-(jpegMixer->jpegTime)>1) // artificial limitation to prevent overload: no more than 1 frame per second
  {
    ...
    {
      width=QazMeet::vmcfg.vmconf[jpegMixer->GetPositionSet()].splitcfg.mockup_width;
      height=QazMeet::vmcfg.vmconf[jpegMixer->GetPositionSet()].splitcfg.mockup_height;
    }
...
InteractiveHTTP::InteractiveHTTP(QazMeet & _app, PHTTPAuthority & auth)
...
BOOL InteractiveHTTP::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  message << QazMeet::Current().HttpStartEventReading(idx,room);

//PTRACE(1,"!!!!!\tsha1('123')=" << PMessageDigestSHA1::Encode("123")); // sha1 works!! I'll try with websocket in future

  ConferenceManager *cm = QazMeet::Current().GetConferenceManager();
  MCUH323EndPoint & ep = QazMeet::Current().GetEndpoint();
  ...
  if(conference)
  {
    ...
    for (unsigned i=0;i<QazMeet::vmcfg.vmconfs;i++)
    {
      PString split=QazMeet::vmcfg.vmconf[i].splitcfg.Id;
    ...
    for (unsigned i=0;i<QazMeet::vmcfg.vmconfs;i++)
    {
      PString split=QazMeet::vmcfg.vmconf[i].splitcfg.Id;
      split.Replace("\"","\\x22",TRUE,0);
      message << "\"" << split << "\"" << ":[" << QazMeet::vmcfg.vmconf[i].splitcfg.vidnum;
      for(unsigned j=0;j<QazMeet::vmcfg.vmconf[i].splitcfg.vidnum; j++)
      {
        VMPCfgOptions & vo=QazMeet::vmcfg.vmconf[i].vmpcfg[j];
        message << ",[" << vo.posx << "," << vo.posy << "," << vo.width << "," << vo.height << "," << vo.border << "," << vo.label_mask << "]";
      }
      message << "]";
      if(i+1<QazMeet::vmcfg.vmconfs) message << ",";
    }
    message << "};\n"
        << conferenceOpts
        << "p.build_page();\n"
  ...
  while(server.Write((const char*)message,message.GetLength()))
  {
    server.flush();
    int cnt=0;
    message = QazMeet::Current().HttpGetEvents(idx,room);
...
SelectRoomPage::SelectRoomPage(QazMeet & _app, PHTTPAuthority & auth)
...
BOOL SelectRoomPage::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  if(nextRoom.IsEmpty()) nextRoom = QazMeet::Current().GetDefaultRoomName();
...
{
  PString room = data("room");
  msg << QazMeet::Current().GetEndpoint().SetRoomParams(data);
  return TRUE;
}

///////////////////////////////////////////////////////////////

RecordsBrowserPage::RecordsBrowserPage(QazMeet & _app, PHTTPAuthority & auth)
...
BOOL RecordsBrowserPage::OnGET (PHTTPServer & server, const PURL &url, const PMIMEInfo & info, const PHTTPConnectionInfo & connectInfo)
{
  ...
  {
    PString filePathStr = QazMeet::Current().vr_ffmpegDir + PATH_SEPARATOR + data("getfile");
  ...
  PString dir=QazMeet::Current().vr_ffmpegDir;
  ...
  else if(data.Contains("deleteRecordConfirmed"))
  {
    ...
    {
      unlink(QazMeet::Current().vr_ffmpegDir + PATH_SEPARATOR + wtd);
