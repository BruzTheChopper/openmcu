...
void ResizeYUV420P(const void * _src, void * _dst, unsigned int sw, unsigned int sh, unsigned int dw, unsigned int dh)
{
  ...
  else if(scaleFilterType >= 1 && scaleFilterType <= 3)
  {
    ...
    /* filtering */ (libyuv::FilterMode)QazMeet::GetScaleFilter(scaleFilterType)
    );
  }
#endif
#if USE_SWSCALE
  else if(scaleFilterType >= 4 && scaleFilterType <= 14)
  {
    struct SwsContext *sws_ctx = sws_getContext(sw, sh, AV_PIX_FMT_YUV420P,
                                                ...
                                                QazMeet::GetScaleFilter(scaleFilterType), NULL, NULL, NULL);
    if(sws_ctx == NULL)
    {
  ...
  {
    PWaitAndSignal m(QazMeet::Current().videoResizeDeltaTSCMutex);
    QazMeet::Current().videoResizeDeltaTSCSum += (unsigned long)(rdtsc()-TSC0);
    QazMeet::Current().videoResizeDeltaTSCCounter++;
    time_t t0 = time(NULL);
    if(t0 - QazMeet::Current().videoResizeDeltaTSCReportTime >= 3)
    {
      PStringStream msg;
      msg << "resize_timing("
        << std::dec << (QazMeet::Current().videoResizeDeltaTSCSum / QazMeet::Current().videoResizeDeltaTSCCounter)
        << ")";
      QazMeet::Current().HttpWriteCmd(msg);
      QazMeet::Current().videoResizeDeltaTSCSum = 0;
      QazMeet::Current().videoResizeDeltaTSCCounter = 0;
      QazMeet::Current().videoResizeDeltaTSCReportTime = t0;
    }
  }
